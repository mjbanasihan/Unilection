...
if (!empty($search)) {
    ...
    $sql .= " WHERE ID LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR 
                    admin_id LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR 
                    admin_lastn LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR 
                    admin_firstn LIKE '%" . mysqli_real_escape_string($conn, $search) . "%'";
}
...
<body>
    ...
    <div>
            ...
            <table>
                ...
                if (!empty($search)) {
                    // Sanitize input and add WHERE clause for search
                    $sql .= " WHERE admin_id LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR
                                    admin_lastn LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR 
